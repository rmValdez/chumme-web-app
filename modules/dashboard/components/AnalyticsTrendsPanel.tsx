...
export function AnalyticsTrendsPanel({ data, isLoading, isDarkMode }: Props) {
  ...
  const platformEntries: [string, number][] = Array.isArray(platforms)
    ? platforms.map((p) => {
        ...
        const platform = typeof rawPlatform === "string" ? rawPlatform : String(rawPlatform);
  ...
  console.log("[trends] platforms[0]:", Array.isArray(platforms) ? platforms[0] : undefined);
  ...
  const scoutSuccessValue =
    typeof scoutSuccess === "number"
      ...
      : scoutSuccess &&
          ...
          typeof (scoutSuccess as { discoveredTalents7d?: unknown }).discoveredTalents7d === "number"
        ? (scoutSuccess as { discoveredTalents7d: number }).discoveredTalents7d
        : undefined;

  const scoutSuccessIsRate = scoutSuccessValue !== undefined && scoutSuccessValue >= 0 && scoutSuccessValue <= 1;
      ...
      <div className={panel}>
        ...
        {!isLoading && data?.platforms && (
          <div className="space-y-4">
            ...
            <div className={`pt-2 text-xs ${isDarkMode ? "text-gray-500" : "text-gray-400"}`}>
              {scoutSuccessIsRate ? "Scout success rate:" : "Scout success (7d):"}{" "}
        ...
        {!isLoading && data?.trend?.length && (
          <div>
            ...
            <div className="flex justify-between mt-2">
              {data.trend.map((point, i) => (
                <span key={i} className={`text-xs ${isDarkMode ? "text-gray-600" : "text-gray-400"}`}>
