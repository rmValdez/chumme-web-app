...
const getStatus = (release: APKRelease): "Latest" | "Stable" | "Untagged" => {
  if (release.isLatest) return "Latest";
  if (release.isStable) return "Stable";
  return "Untagged";
};
...
export const APKDownloadPage = ({ isDark }: APKDownloadPageProps) => {
  ...
  const { messages, dismiss, showDownload, showSuccess, showError } = useSnackbar();
  ...
  const totalDownloads = releases?.reduce((sum, r) => sum + (r.downloadCount ?? 0), 0) ?? 0;
  const totalVersions = releases?.length ?? 0;
  const lastUpdated = releases?.[0]?.updatedAt
    ? new Date(releases[0].updatedAt).toLocaleDateString("en-US", { month: "short", day: "numeric" })
    : "—";
    ...
    >
      ...
      {uploadMutation.isPending && (
        <motion.div
          ...
          animate={{ opacity: 1, y: 0 }}
          className={`mb-6 p-4 rounded-xl border flex items-center gap-4 ${isDark ? "bg-gray-800/80 border-gray-700/50" : "bg-white border-gray-200"
            }`}
        >
          <motion.div
            animate={{ rotate: 360 }}
            ...
            className="shrink-0"
          >
            <Upload className={`w-5 h-5 ${isDark ? "text-[#EF88AD]" : "text-[#A53860]"}`} />
          </motion.div>
          <div className="flex-1 min-w-0">
            <p className={`text-sm font-medium mb-1.5 ${isDark ? "text-white" : "text-gray-900"}`}>
              Uploading APK…
            </p>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-8">
        {[
          { icon: Smartphone, label: "Latest Version", value: latestRelease?.versionName ?? "—" },
      ...
      >
        ...
        <div>
          ...
          <p className={`text-sm mt-1 ${isDark ? "text-blue-400" : "text-blue-700"}`}>
            ...
            Sources&quot; in your Android device settings. APK files are for testing
            and distribution purposes only.
          </p>
        </div>
      </motion.div>
      ...
      <div className="space-y-4 mb-8">
        {isLoading && (
          <div className={`p-8 rounded-xl border text-center ${isDark ? "bg-gray-800/80 border-gray-700/50 text-gray-400" : "bg-white border-gray-200 text-gray-500"}`}>
            Loading releases…
          </div>
        )}
        {isError && (
          <div className={`p-8 rounded-xl border text-center ${isDark ? "bg-red-900/20 border-red-800/40 text-red-400" : "bg-red-50 border-red-200 text-red-600"}`}>
            Failed to load APK releases. The backend endpoint may not be available yet.
          </div>
        )}
        ...
        {(releases ?? []).map((release) => {
          ...
          const changesList: string[] = Array.isArray(release.whatIsNew)
            ...
            : typeof (release as any).releaseNotes === "string"
            ? (release as any).releaseNotes.split("\n").filter((c: string) => c.trim())
            : [];
              ...
              <div className="flex items-start justify-between px-8 py-6 gap-6">
                ...
                <div className="flex flex-col gap-3 flex-1 min-w-0">
                  ...
                  {changesList.length > 0 && (
                    <div className="mt-1">
                      <p className="text-white/80 text-sm font-semibold mb-1.5">What's New:</p>
                ...
                <div className="flex flex-col gap-2 min-w-[160px]">
                  <button
                    onClick={() => handleDownload(release.id, release.versionName)}
                  ...
                  {!release.isLatest && (
                    ...
                    >
                      {loadingAction[release.id] === "latest" ? "Updating..." : "Mark as Latest"}
                    </button>
                  )}
                  ...
                  {!release.isStable && (
                    <button
                      ...
                      className="px-4 py-2.5 rounded-xl text-sm font-semibold border border-white/20 text-white/70 hover:bg-white/10 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
                    >
                      {loadingAction[release.id] === "stable" ? "Updating..." : "Mark as Stable"}
                    </button>
                  )}
