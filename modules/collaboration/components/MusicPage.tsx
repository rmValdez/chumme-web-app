  ...
  return (
    <div>
      <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.5 }} className="mb-8">
        <h1 className={`text-3xl font-bold mb-2 ${isDark ? "text-white" : "text-gray-900"}`}>Music Management</h1>
        <p className={isDark ? "text-gray-400" : "text-gray-600"}>Manage music library for collaboration rooms</p>
      </motion.div>
      ...
      <div className={`rounded-lg border overflow-hidden ${isDark ? "bg-gray-800 border-gray-700" : "bg-white border-gray-200"}`}>
        <table className="w-full">
          <thead className={isDark ? "bg-gray-900" : "bg-gray-50"}>
            <tr>
              {["Title", "Artist", "Album", "Genre", "Duration", "Status", "Actions"].map((h) => (
          ...
          <tbody className={`divide-y ${isDark ? "divide-gray-700" : "divide-gray-200"}`}>
            {mockMusicSongs.map((song) => (
              <tr key={song.id} className={isDark ? "hover:bg-gray-700" : "hover:bg-gray-50"}>
                ...
                <td className={`px-6 py-4 ${isDark ? "text-gray-300" : "text-gray-700"}`}>{song.album}</td>
                <td className={`px-6 py-4 ${isDark ? "text-gray-300" : "text-gray-700"}`}>{song.genre}</td>
                ...
                <td className="px-6 py-4">
                  <div className="flex gap-2">
                    ...
                    <button className={`p-2 rounded-lg ${isDark ? "hover:bg-yellow-500/20" : "hover:bg-yellow-50"}`}>
                      {song.status === "Active"
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
              ...
              <div className="flex items-center justify-between mb-6">
                <h3 className={`text-xl font-bold ${isDark ? "text-white" : "text-gray-900"}`}>Add Song</h3>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <select className={selectClass}>
                    ...
                    <option>Pop</option><option>K-Pop</option><option>Hip-Hop</option>
                    <option>R&B</option><option>Rock</option><option>Jazz</option>
                  </select>
                </div>
