      ...
      <div className={`rounded-lg border overflow-hidden ${isDark ? "bg-gray-800 border-gray-700" : "bg-white border-gray-200"}`}>
        <table className="w-full">
          <thead className={isDark ? "bg-gray-900" : "bg-gray-50"}>
            <tr>
              {["Name", "Category", "Subcategory", "Status", "Actions"].map((h) => (
          ...
          <tbody className={`divide-y ${isDark ? "divide-gray-700" : "divide-gray-200"}`}>
            {topics.map((topic) => (
              <tr key={topic.id} className={isDark ? "hover:bg-gray-700" : "hover:bg-gray-50"}>
                ...
                <td className={`px-6 py-4 ${isDark ? "text-gray-300" : "text-gray-700"}`}>{topic.categoryName}</td>
                <td className={`px-6 py-4 ${isDark ? "text-gray-300" : "text-gray-700"}`}>{topic.subcategoryName}</td>
                <td className="px-6 py-4">
                  <span className={`px-2 py-1 text-xs rounded-full ${isDark ? "bg-gray-700 text-gray-300" : "bg-gray-100 text-gray-700"}`}>Active</span>
                </td>
                <td className="px-6 py-4">
                  <div className="flex gap-2">
                    <button onClick={() => setModalData({ type: "edit-topic", item: { id: topic.id, name: topic.name, description: topic.note ?? "", category: topic.categoryName, subcategory: topic.subcategoryName } })} className={`p-2 rounded-lg ${isDark ? "hover:bg-gray-600" : "hover:bg-gray-100"}`}>
                      <Edit className={`w-4 h-4 ${isDark ? "text-gray-400" : "text-gray-600"}`} />
                    </button>
                    <button onClick={() => setModalData({ type: "delete-confirm", item: { id: topic.id, name: topic.name, itemType: "topic" } })} className={`p-2 rounded-lg ${isDark ? "hover:bg-red-900/30" : "hover:bg-red-50"}`}>
